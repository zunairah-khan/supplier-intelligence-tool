...
const UserList = ({setTeam,team}) => {
  ...
  return (
    <div>
        <p className='text-gray-700'> Assignees: </p>
        ...
        <div className='relative mt-1'>
          ...
          >
             ...
             {data?.map((user,index)=> (
                <ListboxOption key={index} className={({active})=> `relative cursor-default elect-none py-2 pl-10 pr-4 ${
                  active ? "bg-blue-100 text-blue-900" : "text-gray-900"
                }`}
                value={user}>
                  {({selected})=> (
                    <>
                    <div className={clsx("flex items-center gap-2 truncate", selected ?
